...
function ChatMessage({ speaker, label, color, content, time }: { speaker: string; label: string; color: string; content: string; time?: string }) {
  ...
  return (
    <div className="mb-2 pl-3" style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <div className="text-[10px] text-term-fg mt-0.5 whitespace-pre-wrap">{truncated.substring(0, 2000)}{truncated.length > 2000 ? "..." : ""}</div>
    </div>
  );
}
  ...
  return (
    <div className="p-3 flex flex-col gap-3 h-full overflow-hidden">
      ...
      <div className="flex gap-3 flex-1 min-h-0">
        ...
        {selectedCycle && (
          <div className="w-[55%] overflow-y-auto space-y-2">
            {conversation ? (
              <>
                <div className="panel">
                  <div className="panel-header">
                ...
                <div className="panel">
                  ...
                  <div className="p-2">
                    <textarea
                      ...
                      className="w-full bg-term-bg border border-term-border rounded px-2 py-1.5 text-[11px] text-term-fg font-mono resize-y min-h-[140px] focus:border-blue-500 outline-none placeholder:text-term-muted/50"
                      rows={10}
                    />
                ...
                {conversation.stages?.filter((s: any) => s.stage === "Analysts" || s.sub_stage?.includes("analyst")).length > 0 && (
                  <div className="panel">
                    ...
                    <div className="p-3 bg-term-bg/50 overflow-auto min-h-[140px] max-h-[300px]">
                      {conversation.stages
